...
from miasm.os_dep.win_64_structs import TEB64, ListEntry64, NT_TIB64, PEB64, \
    PEB64_LDR_DATA, LdrDataEntry64
...
def build_teb_64(jitter, teb_address):
    """
    Build TEB information for 64bit arch using following structure:

    @jitter: jitter instance
    @teb_address: the TEB64 address
    """
    # Only allocate space for ExceptionList/ProcessEnvironmentBlock/Self
    jitter.vm.add_memory_page(
        teb_address,
        PAGE_READ | PAGE_WRITE,
        b"\x00" * NT_TIB64.get_offset("StackBase"),
        "TEB.NtTib.ExceptionList"
    )
    jitter.vm.add_memory_page(
        teb_address + NT_TIB64.get_offset("Self"),
        PAGE_READ | PAGE_WRITE,
        b"\x00" * (NT_TIB64.sizeof() - NT_TIB64.get_offset("Self")),
        "TEB.NtTib.Self"
    )
    jitter.vm.add_memory_page(
        teb_address + TEB64.get_offset("ProcessEnvironmentBlock"),
        PAGE_READ | PAGE_WRITE,
        b"\x00" * (
            TEB64.get_offset("LastErrorValue") -
            TEB64.get_offset("ProcessEnvironmentBlock")
        ),
        "TEB.ProcessEnvironmentBlock"
    )
    Teb = TEB64(jitter.vm, teb_address)
    Teb.NtTib.ExceptionList = DEFAULT_SEH
    Teb.NtTib.Self = teb_address
    Teb.ProcessEnvironmentBlock = peb_address

def build_peb_64(jitter, peb_address):
    """
    Build PEB information for 64bit arch using following structure:

    @jitter: jitter instance
    @peb_address: the PEB64 address
    """

    if main_pe:
        offset, length = 0xC, 8
    else:
        offset, length = 0x18, 0
    length += 0x0C
    jitter.vm.add_memory_page(
        peb_address + offset,
        PAGE_READ | PAGE_WRITE,
    ...
    Peb = PEB64(jitter.vm, peb_address)
    if main_pe:
        Peb.ImageBaseAddress = main_pe.NThdr.ImageBase
    Peb.Ldr = peb_ldr_data_address


def build_ldr_data_64(jitter, modules_info):
    """
    Build Loader information for 64bit arch using following structure:

    +0x000 Length                          : Uint4B
    +0x004 Initialized                     : UChar
    +0x008 SsHandle                        : Ptr64 Void
    +0x010 InLoadOrderModuleList           : _LIST_ENTRY_64
    +0x020 InMemoryOrderModuleList         : _LIST_ENTRY_64
    +0x030 InInitializationOrderModuleList         : _LIST_ENTRY_64
    # dummy dll base
    +0x040 DllBase : Ptr32 Void
    ...
    ldrdata = PEB64_LDR_DATA(jitter.vm, addr)
    ...
    if main_pe:
        ...
        ldrdata.InMemoryOrderModuleList.flink = main_addr_entry + \
            LdrDataEntry64.get_type().get_offset("InMemoryOrderLinks")
        ldrdata.InMemoryOrderModuleList.blink = 0

    if ntdll_pe:
        ldrdata.InInitializationOrderModuleList.flink = ntdll_addr_entry + \
            LdrDataEntry64.get_type().get_offset("InInitializationOrderLinks")
        ldrdata.InInitializationOrderModuleList.blink = 0

    # Add dummy dll base
    offset = PEB64_LDR_DATA.get_offset("InInitializationOrderModuleList") + ListEntry64.sizeof()
    jitter.vm.add_memory_page(peb_ldr_data_address + offset + 0x10,
                              PAGE_READ | PAGE_WRITE, pck64(0),
...
def create_modules_chain(jitter, name2module):
    ...
    for i, (fname, pe_obj) in enumerate(viewitems(name2module), 1):
        ...
        # Allocate a partial LdrDataEntry64 (0-Flags)
        jitter.vm.add_memory_page(
            addr,
            PAGE_READ | PAGE_WRITE,
            b"\x00" * LdrDataEntry64.get_offset("Flags"),
            "Module info %r" % bname_str
        )
        
        LdrEntry = LdrDataEntry64(jitter.vm, addr)
...
def set_link_list_entry(jitter, loaded_modules, modules_info, offset):
    for i, module in enumerate(loaded_modules):
        ...
        if i == 0:
            prev_module_entry = peb_ldr_data_address + 0x18             # will point to PEB_LDR_DATA.InLoadOrderModuleList.Blink (x64)
        if i == len(loaded_modules) - 1:
            next_module_entry = peb_ldr_data_address + 0x18             # will point to PEB_LDR_DATA.InLoadOrderModuleList.Blink (x64)
        jitter.vm.set_mem(cur_module_entry + offset,
                          (pck64(next_module_entry + offset) +
                           pck64(prev_module_entry + offset)))
...
def fix_InLoadOrderModuleList(jitter, modules_info):
    ...
    set_link_list_entry(jitter, loaded_modules, modules_info, 0x0) # offset of InLoadOrderLinks is 0x0 in 64-bits
...
def fix_InMemoryOrderModuleList(jitter, modules_info):
    ...
    set_link_list_entry(jitter, loaded_modules, modules_info, 0x10) # offset of InMemoryOrderLinks is 0x10 in 64-bits
...
def fix_InInitializationOrderModuleList(jitter, modules_info):
    ...
    set_link_list_entry(jitter, loaded_modules, modules_info, 0x20) # offset of InInitializationOrderLinks is 0x20 in 64-bits
...
def init_seh_64(jitter):
    ...
    build_teb_64(jitter, tib_ad)
    build_peb_64(jitter, peb_address)
    ...
    build_ldr_data_64(jitter, modules_info)
