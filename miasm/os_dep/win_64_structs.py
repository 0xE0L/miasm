...
class UnicodeString64(MemStruct): # add UNICODE_STRING struct for Win64
    fields = [
        ("length", Num("H")), # 2 bytes
        ("maxlength", Num("H")), # 2 bytes
        ("padding", Num("I")), # 4 bytes to simulate padding we have in 64-bits
        ("data", Ptr("<Q", Str("utf16"))), # 8 bytes
    ]

class LdrDataEntry64(MemStruct): # add LdrDataEntry struct for Win64

    """
    +0x000 InLoadOrderLinks : _LIST_ENTRY
    +0x010 InMemoryOrderLinks : _LIST_ENTRY
    +0x020 InInitializationOrderLinks : _LIST_ENTRY
    +0x030 DllBase : Ptr32 Void
    +0x038 EntryPoint : Ptr32 Void
    ...
    +0x070 SectionPointer : Ptr32 Void
    [...]
    """
    ...
    fields = [
        ...
        ("Flags", Array(Num("B"), 4)), # simulating this struct after member "Flags" shouldn't be necessary for 99% cases
    ]
